...
class Attendance:
    ...
    def __init__(self,root):
        ...
        # showAll_btn=Button(search_frame,command=self.search_data,text="Show All",width=8,font=("verdana",12,"bold"),fg="white",bg="navyblue")
    ...
    def update_data(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(host="localhost",username="root",password="963852741",database="attendace")
                    mycursor = conn.cursor()
                    mycursor.execute("update student set std_id=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
